...
import { FiFilter, FiArrowRight, FiTarget, FiEye } from 'react-icons/fi';
              ...
              {works.map((work) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <div className={`absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/50 to-transparent transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                      <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          {work.tags?.map((tag, idx) => (
                            <span key={idx} className="text-xs px-2 py-1 bg-white/10 backdrop-blur-sm rounded-full text-gray-200">{tag}</span>
                          ))}
                        </div>
                        <div className="flex items-center text-white gap-2">
                          <span className="text-sm font-medium">{isActive ? 'Tap again to view' : 'View Case Study'}</span>
                          <FiArrowRight className={`transition-transform ${isActive ? 'translate-x-1' : 'group-hover:translate-x-2'}`} />
                        </div>
                      </div>
                    </div>
                    <div className="absolute top-6 left-6">
                      <span className="px-4 py-2 bg-white/90 backdrop-blur-sm rounded-full text-sm font-medium text-gray-900">{work.year}</span>
                    </div>
      ...
      <section className="py-20 md:py-24 bg-gray-50">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <span className="inline-block text-xs font-bold tracking-[0.3em] text-gray-800 uppercase mb-3">Our Impact</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <div key={index} className="process-step text-center group">
                <div className="text-5xl font-black text-gray-300 mb-4 group-hover:text-gray-400 transition-colors">{item.number}</div>
          ...
          <div className="relative p-12 md:p-16 rounded-3xl overflow-hidden bg-gray-900">
            ...
            <div className="relative z-10 text-center text-white">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="/contact" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-white text-gray-900 rounded-full font-bold hover:shadow-[0_0_40px_rgba(255,255,255,0.2)] transition-all group">
                  <span>Start Your Project</span>
                  <FiArrowRight className="group-hover:translate-x-1 transition-transform" />
                </a>
                <a href="/services" className="inline-flex items-center justify-center gap-2 px-8 py-4 border-2 border-white/30 rounded-full font-bold hover:bg-white/10 hover:border-white transition-all text-white">
                  Explore Services
                </a>
              </div>
            </div>
