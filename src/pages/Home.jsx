...
import { FiArrowDown, FiPlus, FiMinus, FiArrowRight } from 'react-icons/fi';
...
function ClientTrack({ names, direction = 'left', speed = 30 }) {
  ...
  useEffect(() => {
    ...
    const anim = gsap.fromTo(el,
      { x: direction === 'left' ? 0 : -w },
      { x: direction === 'left' ? -w : 0, duration: speed, repeat: -1, ease: 'none' }
    );
    return () => anim.kill();
  }, [direction, speed]);
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray('.work-cell').forEach((el, i) =>
        gsap.fromTo(el, { scale: 0.93, opacity: 0 }, {
          scale: 1, opacity: 1, duration: 0.65, delay: i * 0.06, ease: 'power3.out',
          scrollTrigger: { trigger: el, start: 'top 89%', toggleActions: 'play none none none' },
        })
      );
      if (faqTitleRef.current)
        gsap.fromTo(faqTitleRef.current, { x: -55, opacity: 0 }, {
  ...
  return (
    <div ref={containerRef} className="bg-[#0a0a0a] overflow-x-hidden">
      ...
      <section className="relative w-full bg-black overflow-hidden" style={{ height: '100dvh', minHeight: 560 }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center z-10 text-center px-4">
          <div className="overflow-hidden mb-5">
            <h1 className="h-title font-black text-white leading-none tracking-tighter" style={{ fontSize: 'clamp(2.8rem, 11vw, 11rem)' }}>
              SECOND WAVE.
            </h1>
          </div>
          ...
          <Link to="/contact"
            className="h-btn inline-flex items-center gap-2.5 text-white font-black rounded-full px-9 py-3.5 text-xs sm:text-sm tracking-widest uppercase transition-all hover:scale-105"
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
          <div>
            ...
            <h2 className="ru font-black text-white leading-tight mb-5" style={{ fontSize: 'clamp(1.8rem, 3.8vw, 3rem)' }}>
              Digital Marketing<br />Agency in Kochi<br />
            </h2>
            ...
            <Link to="/works"
              className="inline-flex items-center gap-2.5 text-white font-black rounded-full px-7 py-3.5 text-xs tracking-widest uppercase transition-all group hover:opacity-90 hover:scale-105"
              style={{ background: ACCENT }}>
          ...
          <div className="grid grid-cols-2 gap-3 ru-stagger">
            {[
              ...
              <div key={i} className="rounded-2xl p-5 sm:p-6 border transition-all hover:scale-[1.02]"
                ...
                <div className="font-black text-white leading-none mb-1.5" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.2rem)' }} data-count={s.count} data-suffix={s.suffix}>{s.count}{s.suffix}</div>
      ...
      {/* 4 ── IMAGE STRIP — DO Studio style */}
      <section className="bg-[#0a0a0a] px-4 sm:px-6 md:px-8 py-4 sm:py-6">
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-3 gap-3 sm:gap-4">
            {[
              { src: '/branding.jpg', label: 'Branding'    },
              { src: '/perfo.jpg',    label: 'Performance' },
              { src: '/bran.avif',    label: 'Design'      },
            ].map((item, i) => (
              <Link key={i} to="/works"
                className="strip-card group relative overflow-hidden rounded-2xl sm:rounded-3xl block"
          ...
          <div className="mt-3 sm:mt-4">
            <Link to="/works"
              className="strip-card group relative overflow-hidden rounded-2xl sm:rounded-3xl block"
              style={{ aspectRatio: '21/7', background: '#1a1a1a', minHeight: 180 }}>
              <img
                src="/mark.png"
                alt="Marketing"
                className="absolute inset-0 w-full h-full object-cover object-center transition-transform duration-500 group-hover:scale-[1.03]"
              ...
              <div className="absolute inset-0 flex items-center px-8 sm:px-12">
                <div>
                  <p className="text-white/45 text-[10px] tracking-[0.3em] uppercase mb-2">Featured Work</p>
                  <h3 className="text-white font-black leading-tight mb-5" style={{ fontSize: 'clamp(1.5rem, 3vw, 2.5rem)' }}>
                    Kochi's Creative<br />Marketing Agency
                  </h3>
                  <span className="inline-flex items-center gap-2 text-white font-black text-[10px] sm:text-xs tracking-widest uppercase px-5 py-2.5 rounded-full border border-white/30 group-hover:bg-white group-hover:text-black transition-all duration-300">
                    View Works <FiArrowRight size={11} />
                  </span>
                </div>
              </div>
            </Link>
      ...
      {/* 5 ── SERVICES — DO Studio 3-col + spinning logo */}
      <section className="bg-[#0a0a0a] py-14 sm:py-20 md:py-24 px-4 sm:px-6 md:px-8">
        <div className="max-w-5xl mx-auto">
      ...
      <section className="bg-[#0f0f0f] py-14 sm:py-20 md:py-24 px-4 sm:px-6 md:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3" style={{ gridTemplateRows: 'auto auto' }}>
            ...
            <div className="work-cell row-span-2 overflow-hidden rounded-2xl relative flex flex-col" style={{ background: ACCENT }}>
            ...
            <Link to="/works" className="work-cell group relative overflow-hidden rounded-2xl bg-white/4" style={{ aspectRatio: '1/1' }}>
              <img src="/bran.avif" alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 translate-y-1 group-hover:translate-y-0 transition-transform duration-300">
      ...
      <section className="bg-[#0a0a0a] py-12 sm:py-16 overflow-hidden">
        ...
        <div className="text-center mt-7">
          <button className="border border-white/10 text-white/28 font-semibold text-[11px] tracking-widest uppercase px-7 py-2.5 rounded-full hover:border-white/20 hover:text-white/45 transition-all">
            Load More
          </button>
        </div>
      </section>
      ...
      <section className="bg-[#0f0f0f] py-12 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="rounded-2xl border border-white/8 p-7 sm:p-10 relative overflow-hidden transition-opacity duration-300"
            style={{ opacity: testiAnim ? 0 : 1, background: '#141414' }}>
            <span className="absolute font-black select-none leading-none pointer-events-none"
              style={{ fontSize: 'clamp(5rem, 12vw, 9rem)', color: `${ACCENT}08`, top: '-0.8rem', left: '-0.2rem' }}>"</span>
            <div className="relative z-10">
              <p className="font-black text-white leading-snug mb-7" style={{ fontSize: 'clamp(1rem, 2.5vw, 1.6rem)', lineHeight: 1.4 }}>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] xl:grid-cols-[340px_1fr] gap-8 lg:gap-16 items-start">
            <div ref={faqTitleRef} className="lg:sticky lg:top-24">
              ...
              <p className="text-white/22 text-xs sm:text-sm leading-relaxed mt-4 max-w-[260px]">
                Everything you need to know about working with SecondWave, Kochi.
              </p>
              <Link to="/contact"
                className="inline-flex items-center gap-2 mt-5 text-white font-black rounded-full px-5 py-2.5 text-xs tracking-widest uppercase transition-all hover:opacity-90"
                style={{ background: ACCENT }}>
      ...
      <section ref={ctaRef} className="bg-[#0f0f0f] py-14 sm:py-20 md:py-28 px-4 sm:px-6 md:px-8">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <form onSubmit={onSubmit} className="cta-body max-w-[290px] mx-auto flex flex-col gap-3">
            ...
            <button type="submit"
              className="w-full text-white font-black rounded-full py-3.5 text-sm tracking-widest uppercase transition-all hover:opacity-90 hover:scale-[1.02]"
              style={{ background: ACCENT }}>
              Get Started
            </button>
          </form>
...
function ClientTrackWhite({ names, direction = 'left', speed = 30 }) {
  const ref = useRef(null);
  useEffect(() => {
    if (!ref.current) return;
    const el = ref.current;
  ...
  return (
    <div className="overflow-hidden w-full">
      <div ref={ref} className="flex items-center w-max">
        {doubled.map((name, i) => (
          <React.Fragment key={i}>
            <span className="font-black uppercase tracking-tight text-black/12 whitespace-nowrap px-5 sm:px-8" style={{ fontSize: 'clamp(1.2rem, 3vw, 2.2rem)' }}>{name}</span>
            <span className="font-black select-none text-base" style={{ color: ACCENT, opacity: 0.45 }}>✦</span>
          </React.Fragment>
        ))}
      </div>
    </div>
