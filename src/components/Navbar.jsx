  ...
  return (
    <nav
      className={`navbar fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-[#0a0a0a]/98 backdrop-blur-xl border-b border-white/6 h-[58px]'
          : 'bg-black/30 backdrop-blur-md h-[64px]'
      }`}
    >
      <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-8 h-full flex items-center justify-between relative">
        ...
        <button
          ...
          className="nav-item md:hidden text-white/45 hover:text-white transition-colors ml-auto"
          aria-label="menu"
        >
      ...
      {isMenuOpen && (
        <div
          className="md:hidden absolute top-full left-0 w-full border-t border-white/8 shadow-2xl"
          style={{ background: '#0a0a0a' }}
        >
          <div className="py-6 flex flex-col px-4">
            {[['/', 'Home'], ['/services', 'Services'], ['/works', 'Works'], ['/about', 'About']].map(
              ([to, label]) => (
                <NavLink
                  key={to}
                  to={to}
